...
use pallet_bridge_headerchain::{
	self as headerchain,
	traits::OnHeadersSubmitted,
};
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		/// Import Aura chain header. Ignores non-fatal errors (like when known
		/// header is provided).
		pub fn import_header(origin, header: Header, receipt: Option<Receipt>) {
			headerchain::Module::<T>::import_header(origin, header, receipt)?
		}
	}
}
